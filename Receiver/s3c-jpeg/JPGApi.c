...
JPEG_ERRORTYPE SsbSipJPEGEncodeExe(int dev_fd, ExifFileInfo *Exif, JPEG_SCALER scaler)
{
	...
	if ((Exif != NULL) && (jCtx->thumbnailFlag == TRUE)){
		...
		if(scaler == JPEG_USE_HW_SCALER) {
			...
			pp_param.src_full_width		= jCtx->encParam->width;
			pp_param.src_full_height	= jCtx->encParam->height;
			...
			pp_param.dst_full_width		= jCtx->thumbEncParam->width;
			pp_param.dst_full_height	= jCtx->thumbEncParam->height;
